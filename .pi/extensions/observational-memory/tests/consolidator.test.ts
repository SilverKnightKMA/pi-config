...
describe("registerConsolidatorTools (scoped to .memory/)", () => {
	...
	// Regression (2026-08-30): model wrote `.memory/foo.md` following the system prompt's
	// "under .memory/" wording, which resolved to root/.memory/foo.md — one level too deep.
	// INDEX.md (rendered from root's top level) then said "No topics yet".
	it("a `.memory/` prefix resolves to the same file as the bare name (no nesting)", async () => {
		await tools.get("write").execute("1", { path: ".memory/auth.md", content: "prefixed write" });
		expect(existsSync(join(memoryRoot, ".memory", "auth.md"))).toBe(false);
		expect(readFileSync(join(memoryRoot, "auth.md"), "utf-8")).toBe("prefixed write");
		const viaPrefix = await tools.get("read").execute("2", { path: "./.memory/auth.md" });
		expect(viaPrefix.content[0].text).toContain("prefixed write");
	});
});
