...
/** Same totals as sumSessionCost, split by worker role for cost attribution in status reports. */
export function sumSessionCostByRole(allEntries: Entry[]): {
	total: RoleCostSummary;
	observer: RoleCostSummary;
	consolidator: RoleCostSummary;
} {
	const observer: RoleCostSummary = { costUsd: 0, runs: 0 };
	const consolidator: RoleCostSummary = { costUsd: 0, runs: 0 };
	for (const entry of allEntries) {
		if (!isCostEntry(entry)) continue;
		const data = entry.data as { costUsd: number; role: string };
		if (data.role === "consolidator") {
			consolidator.costUsd += data.costUsd;
			consolidator.runs += 1;
		} else {
			observer.costUsd += data.costUsd;
			observer.runs += 1;
		}
	}
	const total: RoleCostSummary = {
		costUsd: observer.costUsd + consolidator.costUsd,
		runs: observer.runs + consolidator.runs,
	};
	return { total, observer, consolidator };
}
