	...
	pi.registerCommand("om:consolidate", {
		...
		handler: async (_args: string, ctx: any) => {
			...
			if (runtime.consolidatorInFlight) {
				runtime.timeline.notify("om: consolidation already in progress", "warning");
			...
			if (promote.length === 0) {
				const line = `om: nothing to consolidate (pool ${poolTokens(active).toLocaleString()} tok <= target ${runtime.config.poolTargetTokens.toLocaleString()} tok)`;
				runtime.timeline.notify(line);
