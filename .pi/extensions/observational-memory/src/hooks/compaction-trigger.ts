...
export function registerCompactionTrigger(pi: ExtensionAPI, runtime: Runtime): void {
	pi.on("turn_end", (event: any, ctx: any) => {
		...
		runtime.timeline.notify(`om: context full — compacting (waiting for ${runtime.observersInFlight.size} running observer(s))…`);
		...
		ctx.compact({
			onComplete: () => {
				...
				try {
					...
					runtime.timeline.notify(`om: resume failed — ${msg}`, "error");
