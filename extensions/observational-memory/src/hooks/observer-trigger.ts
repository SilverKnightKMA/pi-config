...
export function evaluateObserverTriggers(pi: ExtensionAPI, runtime: Runtime, ctx: TriggerCtx): void {
	...
	while (runtime.observerSlotsAvailable > 0) {
		...
		if (hasUI) startToastLines.push(`om: observer started — summarizing ~${(slice.tokens / 1000).toFixed(1)}k tok of chat…`);
	}

	if (startToastLines.length > 0) {
		// Paseo timeline first (notify is dropped mid-turn under RPC); TUI keeps its popup.
		runtime.timeline.notify(startToastLines.join("\n"));
		ui?.notify(startToastLines.join("\n"), "info");
	}
...
async function dispatchObserver(
	...
	try {
		...
		const exit = await spawnWorker({ argv, cwd: runtime.memoryRoot, env, stdinData: userText, signal: controller.signal });
		...
		runtime.timeline.notify(
			`om: observer done: ${observations.length} observations from ~${(slice.tokens / 1000).toFixed(1)}k tok · $${(runCost?.costUsd ?? 0).toFixed(4)} this run · session $${cost.costUsd.toFixed(2)} (${cost.runs} runs)`,
		);
		if (ctx.hasUI && ctx.ui) {
			...
			runtime.queueToast(
				`om: observer done: +${observations.length} observations`,
				"info",
		...
		runtime.timeline.notify(`om: observer failed: ${message}`, "error");
