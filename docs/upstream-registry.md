...
| `read-only-mode` | **own design** v1.4.17; v1.4.31 borrowed concepts from `@pify/plan-mode@0.4.2` (plan file + parseSteps + step cursor + control-file approve) | plan-mode 0.4.2 | 2026-09-14 — diff 0.4.2→0.4.9 (issue #23): the 4 borrowed regions are byte-identical; the delta is all TUI (ui-lock FIFO, APPROVE_FRESH, localStamp, promptSnippet), not ported → DROP, ref bumped to 0.4.9 | drift-issue | `npm:@pify/plan-mode@0.4.9` |
| `subagent-types` (spawn_pool) | borrowed concepts from `@pify/swarm@0.6.0` (queue + expect-gate) + `@pify/workflow` (resume/gate) — child-mechanism swapped to our spawn_subagent | swarm 0.6.0 / workflow 0.10.0 | 2026-09-14 — diff (issues #24 #25): swarm 0.6.0→0.9.0 adds a loop-guard (~80 lines of node:crypto) → BORROW (ported under task #60, v1.4.58) + the DAG needs SETTING ASIDE (the pool is flat for now); workflow 0.10.0→0.11.6 gate/resume byte-identical, the delta is all unused consent-gate/ui-lock/worktree-fix → DROP | drift-issue | `npm:@pify/swarm@0.9.0 npm:@pify/workflow@0.11.6` |
| `subagent-types` (safe_bash) | port of `amosblomqvist/pi-subagents` `tools/safe-bash.ts` — a 16-regex deny-list; NO mandatory-deny / allowlist (a gap pointed out by the sandbox-runtime eval of 2026-09-12) | pi-subagents `1f54189` | 2026-09-12 — unchanged | drift-issue | `gh:amosblomqvist/pi-subagents@1f54189` |
