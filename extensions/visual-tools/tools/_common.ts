...
export const CHROME_CANDIDATES = [
  ...
  // Linux: puppeteer's chrome-headless-shell cache
  ...chromeHeadlessShellCandidates(),
]

/** Linux chrome-headless-shell binaries under the user's puppeteer cache. */
function chromeHeadlessShellCandidates(): string[] {
  const home = process.env.HOME ?? "/home/coder"
  const cacheRoot = process.env.PUPPETEER_CACHE_DIR ?? join(home, ".cache", "puppeteer")
  const dir = join(cacheRoot, "chrome-headless-shell")
  ...
  const walk = (d: string) => {
    for (const entry of readdirSync(d, { withFileTypes: true })) {
      ...
      else if (entry.name === "chrome-headless-shell") out.push(p)
    }
  }
  walk(dir)
  return out
...
/** Workspace-scoped rsvg-convert shipped with the extracted lib root. */
export function findRsvgConvert(): string | undefined {
  const candidate = join(extensionDir, "..", "..", "..", "lib", "root", "usr", "bin", "rsvg-convert")
...
/**
 * Workspace-scoped shared libraries (.pi/lib/root) extracted from Debian
 * packages — lets the bundled chrome-headless-shell and rsvg-convert run
 * without system-wide apt installs.
 */
function workspaceLibPaths(): string[] {
  const root = join(extensionDir, "..", "..", "..", "lib", "root", "usr", "lib", "x86_64-linux-gnu")
  const dirs = [root, join(root, "gbm")]
  return existsSync(root) ? dirs : []
}

function augmentedLibraryPath(): string {
  const libs = workspaceLibPaths()
  if (libs.length === 0) return process.env.LD_LIBRARY_PATH ?? ""
  return [...libs, process.env.LD_LIBRARY_PATH ?? ""].filter(Boolean).join(":")
}

/**
 * Self-contained fontconfig shipped in the extracted lib root, so the bundled
 * chrome-headless-shell and rsvg-convert resolve the vendored DejaVu fonts on
 * hosts with no system fonts. Falls back to a plain-fonts config when the
 * workspace one is missing.
 */
function fontconfigFile(): string | undefined {
  const conf = join(extensionDir, "..", "..", "..", "lib", "root", "etc", "fonts", "fonts.conf")
  return existsSync(conf) ? conf : process.env.FONTCONFIG_FILE
}
  ...
  return new Promise((resolveRun) => {
    ...
    const child = spawn(cmd, args, {
      ...
      env: {
        ...
        ...(opts.env ?? {}),
        PATH: augmentedPath,
        LD_LIBRARY_PATH: augmentedLibraryPath() || undefined,
        FONTCONFIG_FILE: fontconfigFile() ?? process.env.FONTCONFIG_FILE,
      },
    })
    let stdout = ""
    let stderr = ""
