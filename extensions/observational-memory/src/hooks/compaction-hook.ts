...
export function registerCompactionHook(pi: ExtensionAPI, runtime: Runtime): void {
	pi.on("session_before_compact", async (event: any, ctx: any) => {
		...
		if (runtime.compactHookInFlight) {
			runtime.timeline.notify("om: another compaction is already in progress; cancelling duplicate", "warning");
		...
		try {
			...
			if (!skip) {
				runtime.timeline.notify("om: waiting for in-flight observers before folding…");
